            ...
            client.registerUser(registerInfo)?.enqueue(object : Callback<User?> {
                ...
                override fun onResponse(call: Call<User?>, response: Response<User?>) {
                    if (!response.isSuccessful){
                        ...
                        //var intent = Intent(Intent.ACTION_VIEW) #PART 2.1 REMOVING IMPLICIT INTENT
                        var intent = Intent(activity, ProductScrollingActivity::class.java) //#Adding EXPLICIT CONSENT to MIMIC Action from ThirdFragment.kt
